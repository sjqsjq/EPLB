...
## 任务7: 256tok vs 2048tok 输入下 sw32 vs sw64 对比 (2026-07-23)

### 数据集
- tok256: 2048条，输入220-290 token（均值253），来自Prover-V1
- tok2048: 2048条，输入精确2048 token（拼接多条Prover-V1 prompt构造），output=1

### 结果（各2轮）

| 配置 | 256tok r1/r2 | 均值 | vs BL | 2048tok r1/r2 | 均值 | vs BL |
|---|---|---|---|---|---|---|
| Baseline | 82.32/82.71 | 82.51 | - | 10.12/10.08 | 10.10 | - |
| OEPLB sw=32 | 93.09/95.30 | 94.19 | +14.2% | 11.70/11.72 | 11.71 | **+15.9%** |
| OEPLB sw=64 | 94.60/95.93 | **95.27** | **+15.5%** | 11.63/11.28 | 11.46 | +13.4% |

### sw32 vs sw64 直接对比

| 输入长度 | sw32 | sw64 | sw32相对sw64 |
|---|---|---|---|
| 256tok | 94.19 | **95.27** | **-1.1%（sw64更好）** |
| 2048tok | **11.71** | 11.46 | **+2.2%（sw32更好）** |

### 结论

**用户的猜想成立：输入越长，更小的sync_window(更频繁的检查)越有利。** 在256tok短输入下sw=64仍然是最优（-1.1%的差距虽然在噪声范围内但方向一致）；在2048tok长输入下sw=32反超sw=64（+2.2%），跟之前所有实验里"sw=64始终最优"的结论出现了分歧——**最优sync_window不是一个固定值，而是跟输入长度相关的**。

这个发现为之前设计的adaptive_window机制提供了一个新的切入角度：不仅可以根据cos_sim（workload shift检测）来动态调整window，还可以根据**当前请求的平均输入长度**来调整——长输入期间用更小的window(32)，短输入期间用更大的window(64)。不过目前2.2%的差异量级还不大，是否值得为此增加额外的自适应复杂度还需要评估。
