...
⚠**The multi-domain +14.0% comes from a deleted dataset** (, 16000 requests) and is not reproducible. A measurement on a different available dataset (, 4400 requests) gave −1.1%, but with fewer requests (4400 vs 16000) and shorter benchmark duration (152s vs 247s) this is **not a same-condition re-test** and cannot falsify the original. If +14.0% stands, its η=118%>100% remains an open anomaly (possible causes: DIAG's {\text{before}}$=1.39 is an underestimate, or {\text{sens}}$ is load-dependent above the L512 calibration; both require a same-dataset (r)$ sweep that is no longer possible). This row is marked ⚠ = **neither confirmed nor falsified**, and is excluded from quantitative claims.
