...
| 235B multi-domain (8-GPU) | 1.704§ | 1.093 | 0.359 | 0.496 | 21.4% | **−1.1%** (re-measured) | **−5%** |
...
**The multi-domain row was re-measured to −1.1% (`driver33.sh`, 2 rounds, CV 0.03%/0.53%), conclusively falsifying the original +14.0%.** DIAG self-reports $r_{\text{before}}$=1.704 (far higher than the 1.39 previously used), giving a theoretical ceiling of 21.4%, but 264 swaps in 152s consume the entire headroom — the same eta gate-A failure as on other heterogeneous workloads. This is consistent with 4-GPU multi-domain (−0.24%) and ShareGPT (+0.14%): **heterogeneous workloads yield ~0 or negative gain under the default decision statistic**. The ceiling exists; the current statistic cannot exploit it. Appendix H shows adaptive decay + gate cuts wasteful swaps by 94%.
...
| Multi-domain (8-GPU 235B) | +14.0%⚠→**−1.1%**(重测) | +12.0%⚠ | — |
...
3. **$f_{\text{sens}}$ is load-dependent.** What this appendix provides are values under a specific benchmark configuration (L256, $O=1$, conc=256). The former anomaly of 118% (early draft 131%) on the 235B multi-domain configuration in §2.4 has been **resolved by re-measurement**: the actual gain is −1.1% ($\eta$=−5%), not +14.0%. The old dataset (`/tmp/exp_data/multidomain_16k.jsonl`) is deleted and the measurement cannot be reproduced; the available `multidomain_v2_out1.jsonl` gives a clean negative result consistent with the heterogeneous-workload pattern.
