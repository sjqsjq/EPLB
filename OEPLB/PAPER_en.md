...
| **30B L512 (4-GPU)** | **1.338** | **1.031** | **0.230** | **0.207** | **6.36%** | −3.8%~+0.5%‖ | **≤8%** |
...
**Condition A (cost gate): the headroom must exceed the swap cost.** Both configurations with swap/headroom $\ge1$ (1.26 and 1.09) collapse to zero $\eta$ — the balancer spends more wall-clock time moving experts than the imbalance was costing in the first place. That is a net loss, and no amount of tuning fixes it.
...
|---|---|---|---|---|
| Default | threshold=1.02 | +0.98% | 26% | 72 |
| +dead zone | threshold=$r_k$=1.099 | +2.26% | 59% | 8 |
| **+dead zone + budget** | threshold=1.099, budget=0.02 | **+3.81%** | **100%** | 8 |
| +dead zone + bias + gate | threshold=1.099, bias, gate=0.5 | +2.20% | 57% | 8 |
