...
**One causal chain — and its verification.** 8-GPU 57B makes 72 decisions/round on a homogeneous dataset, which is anomalously many; the cause is that $r_{\text{before}}=1.218$ is only 11% above $r_k=1.099$ while the threshold 1.02 lies inside the dead zone, so the balancer can never reach its own target and never stops. Verification (`driver26.sh` + `driver27.sh`, four-arm A/B test):
