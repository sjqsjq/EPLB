...
### 观察4：M=W/(1-α)统一与M*闭式——自适应记忆的理论目标

**场景**：从指数衰减累积器A_t=R_t+α·A_{t-1}的偏差-方差分析 + 变点响应延迟联合最优化推导，用同M不同(W,α)吻合实验 + 同session adaptive vs 固定α对比验证。

**核心命题**：W（决策频率/all_reduce开销）与α（遗忘曲线形状）**不独立**——稳态下有效记忆M=W/(1-α)与有效样本量N_eff=M·t̄同时由(W,α)决定，二者只通过M影响偏差（抽样偏差∝1/√N_eff）与方差。故**唯一需按负载调的量是M，而非W或α**。早期实现调W不同步α，等于在无意中移动M、漂移偏差-方差工作点。

**M*闭式**：联合最小化方差代价（M小→bias大→η损失）与变点延迟代价（M大→旧信号残留~M·ln2步才半衰→用错布局服务），设段长L_seg：
M* = √( a·c²·L_seg / (b·β·t̄·γ²·(r−r_k)³·ln2) )
方向预测：M*∝√L_seg（段长→更大M，稳态久需更多数据）；M*∝(r−r_k)^{-3/2}（信噪比高→更小M，少数据够、反应快）。c=0.65·EP（标定），t̄/r/r_k/L_seg均可在线测。

**实证**：(1) 同M=128不同(W,α)吞吐吻合4.8%——M是近似充分统计量；(2) M*∝√L_seg方向预测被d31+d34验证；(3) **同session实测**：adaptive（α=0.5稳态+变点α→0清零+grow/shrink W）在构造A上+9.7%，超过固定α=0.9的+6.4%（swap 56 vs 104但吞吐反低→陈旧性损害>开销节省）——**固定α不必追求，零调参adaptive即为最优**。

**独特角度**：不把"自适应"当启发式黑盒，而是给出其理论目标M*与运行时启发式（grow/shrink W+变点清零）作为M*的离散近似（类比Adam自适应学习率追踪最优lr）的关系。

**结论**：自适应记忆的目标由M*闭式给定，运行时启发式追踪之，固定α不是天花板。
