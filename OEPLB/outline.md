...
4. **adaptive window设计**：α最优U形（高r→α=0.9少swap，中r→α=0快反应，低r→α=0.9少亏），固定α只在一个区域最优。adaptive通过调整M=W/(1-α)自动切换策略+auto-dead-zone从EP幂律自动算停止条件。

### 第8段：章节安排

§2相关工作，§3观察（4个insight），§4框架设计（5个机制），§5实验评估，§6总结。
...
本文的核心区别不在"swap vs重排"，而在**理论驱动的设计**：死区理论告诉"何时停"，增益上界告诉"最多能赚多少"，PD任务结构告诉"prefill-only何时充分"，α U形告诉"为何需要adaptive"。
...
**场景**：4组实验，每组测α=0/0.5/0.9，n=1。

| 实验 | prompt | conc | α=0 | α=0.5 | α=0.9 | 最优 |
|------|--------|------|-----|-------|-------|------|
| 3域(book→prover→book) | 4438tok | 32 | +5.4% | +6.1% | **+9.9%** | 0.9 |
| 6域频繁切换 | 4438tok | 32 | +5.6% | +5.3% | **+12.6%** | 0.9 |
| universal多域 | 1000tok | 32 | -7.4% | -5.0% | **+0.1%** | 0.9 |
| universal多域 | 1000tok | 256 | **+7.7%** | +5.7% | +6.7% | 0 |

**独特角度**：α=0.5从未在4组实验中胜出，但始终在最优点2pp内——是"鲁棒默认值"而非"最优值"。最优取决于benefit-per-swap：高r→收益够大→少swap省开销→α=0.9赢；中r→收益有限→需最大化正确swap→α=0快反应赢；低r→swap无用→少swap少亏→α=0.9赢。这是U形：两端α=0.9，中间α=0。
...
**挑战**：（1）何时停止swap？（2）固定α只在一个区域最优，如何自适应？（3）prefill-only recording何时充分？**思想**：死区理论→auto-dead-zone自动算停止条件；α U形→adaptive window自动切换策略；PD任务结构→prefill-only充分性边界。**框架**（Fig架构图）。
...
- W和α不是独立旋钮，只通过M影响偏差-方差
- 同M=128不同(W,α)吻合4.8%（M是近似充分统计量）
- M_min = c²/(γ(r-r_k))²t̄（方差约束，c=0.65·EP）
- 实践规则：W≥16，若W<16则α=0或α≥0.75
...
- Fig F2：α=0/0.5/0.9 sweep（4组实验U形验证）
...
4. adaptive window：α U形→固定不行→adaptive+auto-dead-zone
