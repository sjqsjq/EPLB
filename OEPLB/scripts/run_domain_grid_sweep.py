#!/usr/bin/env python3
"""Domain spot-check: for each of bookcorpus/hellaswag/humanevalplus x {256,4096},
run baseline + sw16 + sw128 (1 rep each) to check whether the
'smaller window always wins, gain grows with length' pattern generalizes.
"""
import os, subprocess, sys, time, signal

DOMAIN_DIR = "/workspace/EPLB/OEPLB/benchmarks/domain_grid"
...
DOMAINS = ["bookcorpus", "hellaswag", "humanevalplus"]
LENGTHS = [256, 4096]
WINDOWS = [16, 128]  # + baseline
...
def build_configs():
    ...
    for D in DOMAINS:
        for L in LENGTHS:
            configs.append({"label": f"domain_{D}_L{L}_bl", "domain": D, "length": L, "window": None})
            for W in WINDOWS:
                configs.append({"label": f"domain_{D}_L{L}_sw{W}", "domain": D, "length": L, "window": W})
    return configs
...
def run_one(cfg):
    label = cfg["label"]
    dataset = f"{DOMAIN_DIR}/{cfg['domain']}_tok{cfg['length']}_out1.jsonl"
