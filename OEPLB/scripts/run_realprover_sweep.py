#!/usr/bin/env python3
"""Real DeepSeek-Prover-V1 dataset (8192 reqs, ~256tok, conc=256 fixed),
sized to guarantee >=2 swap activations even at the largest window (128).
Runs: baseline + windows {8,16,32,64,128}."""
import os, subprocess, sys, time, signal, json

DATASET = "/workspace/EPLB/OEPLB/benchmarks/comprehensive_grid/L256_O1_realprover_n8192.jsonl"
RESULT_DIR = "/workspace/EPLB/OEPLB/benchmarks/results/realprover"
LOG_DIR = "/workspace/EPLB/OEPLB/benchmarks/logs/realprover"
...
CONFIGS = [{"label": "realprover_bl", "window": None}]
for w in WINDOWS:
    CONFIGS.append({"label": f"realprover_sw{w}", "window": w})
...
def run_one(cfg):
    ...
    bench = subprocess.run(
        ["python3", "run_grid_bench.py", f"realprover/{label}", DATASET, str(CONC)],
    ...
    if os.path.exists(result_path):
        ...
        print(f"  FAIL: no result. stderr={bench.stderr[-500:]}", flush=True)
        return False


def main():
    print(f"=== Real DeepSeek-Prover-V1 sweep: {len(CONFIGS)} configs, conc={CONC}, N=8192 ===", flush=True)
    n_ok, n_fail = 0, 0
