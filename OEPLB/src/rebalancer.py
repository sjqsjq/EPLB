...
def try_build_swap_plan(
    ...
    for _ in range(max_total_ops):
        ...
        if not found_swap:
            # No untried pairwise (hot, cold) combination improved this
            # layer's ratio -- per PAPER.md §3.3, PB-OEPLB is a pure
            # two-mode pairwise planner (max-delta / gap-targeting), with
            # no fallback beyond pairwise swaps. Mark exhausted and let the
            # next-highest-ratio layer compete for the remaining budget.
