...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(
        ...
        # Optional MoE routing distribution recorder (controlled by env vars).
        # Activated only when SGLANG_ROUTING_RECORD=1; otherwise zero overhead.
        # Uses pure-GPU scatter_add_ — safe during CUDA Graph capture.
        try:
            from sglang.srt.layers.moe.fused_moe_triton.routing_logger import (
                maybe_attach_routing_recorder,
            )

            maybe_attach_routing_recorder(self)
        except Exception as _e:  # never break model init because of logging
            logger.warning("attach MoE routing recorder failed: %s", _e)
    ...
    def forward(self, hidden_states: torch.Tensor, topk_output: TopKOutput):
        ...
        # Routing recorder: capture gate's ORIGINAL global expert IDs here,
        # BEFORE dispatcher.dispatch() remaps them to EP-local IDs.
        # Zero overhead when SGLANG_ROUTING_RECORD != 1.
        try:
            from sglang.srt.layers.moe.fused_moe_triton.routing_logger import (
                record_routing_from_forward,
            )
            _topk_ids_for_record = getattr(topk_output, "topk_ids", None)
            if _topk_ids_for_record is not None:
                record_routing_from_forward(self.layer_id, _topk_ids_for_record)
        except Exception:
            pass
