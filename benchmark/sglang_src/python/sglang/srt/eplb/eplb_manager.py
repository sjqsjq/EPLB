...
class EPLBManager:
    ...
    def rebalance(self):
        ...
        if enable_timing:
            ...
            time_end = time.time()
            msg += (
                f" total={time_end - time_start:.3f}s"
                f" stat_collect={time_after_stat - time_start:.3f}s"
                f" algorithm={time_after_algo - time_after_stat:.3f}s"
                f" p2p_transfer={time_end - time_after_algo:.3f}s"
            )
        logger.info(msg)
